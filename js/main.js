...
window.onscroll = () => {
  ...
  for (let y = 0; y < myBoxes.length; y++){
    if (window.scrollY > myBoxes[y].offsetTop && window.scrollY < myBoxes[y].offsetTop + myBoxes[y].offsetHeight) {
      myLinks[y].classList.add("active")
    } else {
      myLinks[y].classList.remove("active")
    }
  }
}
// Counter Nums
